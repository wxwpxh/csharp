...
namespace Manning
{
	namespace MyPhotoAlbum
	{
		...
		public class PhotoAlbum : CollectionBase, IDisposable
		{
			...
			/// <summary>
			/// Internal method for opening a file, optionally ignoring the password
			/// </summary>
			/// <param name="fileName">the album file to open</param>
			/// <param name="ignorePwd">whether to ignore the password</param>
			private void Open(string fileName, bool ignorePwd)
			{
				FileStream fs = new FileStream(fileName,
					FileMode.Open,
					FileAccess.Read);
			...
			/// <summary>
			/// Public Open method that requires password
			/// </summary>
			/// <param name="fileName">the album file to open</param>
			...
			static public PhotoAlbum.AlbumInfo GetInfo(string albumFile)
			{
				return new AlbumInfo(albumFile);
			}

			/// <summary>
			/// The book recommends that AlbumInfo is a structure, so we implement
			/// it as such.  A drawback of this approach is that we cannot return
			/// null when encountering an invalid album.  
			/// As a result, we use the IsValid property for this purpose.
			/// </summary>
			public struct AlbumInfo
			{
				...
				public AlbumInfo(string albumFile)
				{
					try
					{
						using (PhotoAlbum album = new PhotoAlbum())
						{
							album.Open(albumFile, true);

							_isValid = true;
							_title = album.Title;
							_hasPassword = (album.Password != null && album.Password.Length > 0);
							_numPhotos = album.Count;
						}
					}
					catch (Exception)
					{
						// Unable to open album, or some other error
						_isValid = false;

						if (File.Exists(albumFile))
							_title = Path.GetFileNameWithoutExtension(albumFile);
						else
							_title = null;
