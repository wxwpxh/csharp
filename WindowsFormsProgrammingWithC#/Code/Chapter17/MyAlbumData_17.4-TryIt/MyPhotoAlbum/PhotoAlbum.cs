	...
	namespace MyPhotoAlbum
	{
		...
		public class PhotoAlbum : CollectionBase, IDisposable
		{
			...
			public void Dispose()
			{
				...
				// Reset photographer array
				_photographerArray = null;
			}
			...
			public void Save(string fileName)
			{
				...
				// Force photographers list to refresh.
				_photographerArray = null;
			}
			...
			private string[] _photographerArray = null;

			/// <summary>
			/// Get the array of photographers for the album.
			/// </summary>
			/// <returns>the photographers as a string array</returns>
			/// <remarks>
			/// Note that this might be better implemented as a property, instead of
			/// a method.  The book recommends a method, so we stick with this design here.
			/// 
			/// Also note that there is no good way to refresh this list when a new
			/// photographer is assigned to a photograph.  In retrospect, I should
			/// probably have included a PhotoAlbum property in the Photograph class
			/// that would permit a Photograph to reference its containing album.
			/// This feature would be useful here to clear this setting as required.
			/// </remarks>
			public string[] GetPhotographers()
			{
				if (_photographerArray == null)
				{
					StringCollection result = new StringCollection();
					foreach (Photograph p in this)
					{
						string photographer = p.Photographer;
					...
					_photographerArray = new string[result.Count];
					result.CopyTo(_photographerArray, 0);
				}

				return _photographerArray;
			}

			public void ClearPhotographers()
			{
				_photographerArray = null;
			}
