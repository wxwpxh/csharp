...
namespace Manning.MyPhotoAlbum
{
	public class PhotoEditDlg : Manning.MyPhotoAlbum.BaseEditDlg
	{
		...
		private void InitializeComponent()
		{
			...
			this.cmbxPhotographer.KeyDown += new System.Windows.Forms.KeyEventHandler(this.cmbxPhotographer_KeyDown);
		...
		private bool _inTextChanged = false;
		...
		private void cmbxPhotographer_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{
			switch (e.KeyCode)
			{
				case Keys.Back:
					string oldText = cmbxPhotographer.Text;
					int start = cmbxPhotographer.SelectionStart;
					int count = cmbxPhotographer.SelectionLength;
					if (start > 0)
					{
						start --;
						count ++;
					...
					if (count > 1)
					{
						cmbxPhotographer.Text = oldText.Remove(start, count);
						e.Handled = true;
					}
					break;

				case Keys.Enter:
					// Enter should simply un-drop the list
					if (cmbxPhotographer.DroppedDown)
					{
						cmbxPhotographer.DroppedDown = false;
						e.Handled = true;
					}
					break;
			}
