...
namespace MyPhotos
{
	...
	public class ParentForm : System.Windows.Forms.Form
	{
		...
		private System.Windows.Forms.ContextMenu ctxtMenuEmpty;
		...
		private void InitializeComponent()
		{
			...
			this.ctxtMenuEmpty = new System.Windows.Forms.ContextMenu();
			...
			this.toolBarParent.ButtonDropDown += new System.Windows.Forms.ToolBarButtonClickEventHandler(this.toolBarParent_ButtonDropDown);
			...
			// ctxtMenuEmpty
			// 
			this.ctxtMenuEmpty.MenuItems.AddRange(new System.Windows.Forms.MenuItem[] {
																						  this.menuItem1});
			// 
			// menuItem1
			// 
			...
			this.menuItem1.Text = "No active image";
			// 
			// ParentForm
			// 
		...
		private void toolBarParent_ButtonDropDown(object sender, System.Windows.Forms.ToolBarButtonClickEventArgs e)
		{
			if (e.Button == tbbImage)
			{
				MainForm f = ActiveMdiChild as MainForm;

				// Calculate point where menu should appear (in ToolBar coordinates)
				Point p = new Point(e.Button.Rectangle.Left, e.Button.Rectangle.Bottom);

				if (f != null)
				{
					// Determine point in MainForm coordinates
					Point formPt = f.PointToClient(toolBarParent.PointToScreen(p));

					// Have MainForm draw the image menu at this point
					f.ShowImageMenu(formPt);
				}
				else
				{
					// Display the empty context menu
					ctxtMenuEmpty.Show(toolBarParent, p);
				}
			}
		}
