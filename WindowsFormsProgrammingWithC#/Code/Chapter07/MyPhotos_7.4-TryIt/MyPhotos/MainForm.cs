...
namespace MyPhotos
{
	...
	public class MainForm : System.Windows.Forms.Form
	{
		...
		private System.Windows.Forms.MenuItem menuFitWidth;
		...
		private void InitializeComponent()
		{
			...
			this.menuFitWidth = new System.Windows.Forms.MenuItem();
			...
			// menuFitWidth
			// 
			this.menuFitWidth.Index = 2;
			this.menuFitWidth.Text = "Fit to &Width";
			this.menuFitWidth.Click += new System.EventHandler(this.menuImage_ChildClick);
			// 
			// MainForm
			// 
		...
		private void pnlPhoto_Paint(object sender, System.Windows.Forms.PaintEventArgs e)
		{
			if (_album.Count > 0)
			{
				...
				switch (_selectedMode)
				{
					...
					case DisplayMode.FitToWidth:
						// Fit image to width of window (preserve aspect ratio)
						// (Note: must use ClientRectangle here to obtain visible area)
						Rectangle fitRect = photo.ScaleToWidth(pnlPhoto.ClientRectangle);
						
						if (fitRect.Height <= pnlPhoto.ClientRectangle.Height)
						{
							// image fits in display area: no problem
							pnlPhoto.AutoScroll = false;
							g.DrawImage(photo.Image, fitRect);
						}
						else
						{
							// image height goes beyond panel: enable scrolling
							pnlPhoto.AutoScroll = true;

							// Draw image into the "fit" area
							...
							pnlPhoto.AutoScrollMinSize
								= new Size(fitRect.Width, fitRect.Height);
						}
						break;
