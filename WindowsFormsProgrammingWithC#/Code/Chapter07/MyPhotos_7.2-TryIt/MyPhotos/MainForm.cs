...
namespace MyPhotos
{
	...
	public class MainForm : System.Windows.Forms.Form
	{
			...
			if (sender is MenuItem)
			{
				...
				switch (_selectedMode)                                  
				{
					...
					case DisplayMode.StretchToFit:
						// Note: I added a call to this method from the
						// constructor to make sure we initialize these
						// values.  In reality, these setting could
						// appear in the constructor here, but the book
						// said to put them here, so here they are....
		...
		{
			// If the selected mode is not ActualSize, then
			// we need to invalidate the form area to redraw
			// the image at the new size.
			switch (_selectedMode)                                  
			{
				default:
				case DisplayMode.ScaleToFit:
